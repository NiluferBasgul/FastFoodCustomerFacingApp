...
export const getStaticPaths = async ({ locales }) => {
    ...
    for (const slug in cucinaTree.mainMenu[MenuCategoryType.WRAPS]) {
    ...
    return {
        paths,
        fallback: false
        // fallback: true if needed
    }
}

export const getStaticProps = async ({ params: { slug }, locale }) => {
    const item = cucinaTree.mainMenu[MenuCategoryType.WRAPS][slug as FoodSlug];
    try {
        ...
        if (item) {
            ...
            item.menuCategory = MenuCategoryType.WRAPS;
            item.customizations = initialBaseCustomizations;
        }
    } catch (err) {
        console.log('wrap menu item translation error', err);
    }
