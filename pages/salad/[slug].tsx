...
export const getStaticPaths = async ({ locales }) => {
    ...
    for (const slug in cucinaTree.mainMenu[MenuCategoryType.SALADS]) {
...
export const getStaticProps = async ({ params: { slug }, locale }) => {
    const item = cucinaTree.mainMenu[MenuCategoryType.SALADS][slug as FoodSlug];
    try {
        ...
        if (item) {
            item.menuCategory = MenuCategoryType.SALADS;
            item.slug = slug as FoodSlug;
            item.customizations = initialBaseCustomizations;
        }
    } catch (err) {
        console.log('salad menu item translation error', err);
    }
